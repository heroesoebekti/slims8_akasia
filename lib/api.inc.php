...
class api
{
  ...
  public static function loan_load($obj_db,$loan_id){
    $_return = FALSE;
    $i = 0;
    $s_mbr = '';
    $s_mbr .= 'SELECT * ';
    $s_mbr .= 'FROM loan l LEFT JOIN mst_loan_rules mlr ON mlr.loan_rules_id= l.loan_rules_id ';
    $s_mbr .= 'WHERE l.loan_id=\''.$loan_id.'\'';
    $q_mbr = $obj_db->query($s_mbr);
    if (!$obj_db->errno) {
      while ($r_mbr = $q_mbr->fetch_assoc()) {
        $loan_data = $r_mbr;
        $member_data  = self::member_load($obj_db,$r_mbr['member_id'])[0];
        $item_data    = self::item_load($obj_db,$r_mbr['item_code'])[0];
        $biblio_data  = self::biblio_load($obj_db,$item_data['biblio_id'])[0];
        unset($biblio_data['items']);  
        if(!empty($member_data) && !empty($item_data) && !empty($biblio_data)){
        $data = array_merge($loan_data, $member_data,$item_data,$biblio_data);
        $remove_field = array('gmd_id','member_type_id', 'coll_type_id','items','hash', 'uid','mpasswd','loan_rules_id','supplier_id');
        foreach ($remove_field as $key) {
          unset($data[$key]);
        }
        $_return[$i] = $data;
      }
  ...
  public static function item_load($obj_db,$item_code){
    $_return = FALSE;
    $i = 0;
    $s_mbr = '';
    $s_mbr .= 'SELECT *,mct.coll_type_name ';
    $s_mbr .= 'FROM item i LEFT JOIN mst_coll_type mct ON i.coll_type_id=mct.coll_type_id LEFT JOIN mst_supplier ms ON ms.supplier_id=i.supplier_id ';
    $s_mbr .= 'WHERE i.item_code=\''.$item_code.'\'';
    $q_mbr = $obj_db->query($s_mbr);
    if (!$obj_db->errno) {
      while ($r_mbr = $q_mbr->fetch_assoc()) {
        $_return[$i] = $r_mbr;
