...
function biblio_list_format($dbs, $biblio_detail, $n, $settings = array(), &$return_back = array()) {
  global $label_cache, $sysconf, $esClient;
  // init output var
  $output     = '';
  ...
  $hasil = $esClient->get('biblio_search',$biblio_id);
  $modal      = json_encode($hasil);
  $data = $hasil['_source'];

  $title_link = '<div id="mymodal" data-toggle="modal" data-target="#myModal'.$biblio_id.'" style="cursor:pointer;">'.$title.'</div>
  <div class="modal fade" id="myModal'.$biblio_id.'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="myModalLabel">'.$data['gmd_name'].'</h4>
      </div>
      <div class="modal-body" style="font-weight:lighter;">
        '.$title.'
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default slims-button--red" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>';
